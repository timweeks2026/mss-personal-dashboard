// Serverless API backing the Meeting Agenda drawer. Stores the whole
// document (the current working meeting's items/punch list, plus a history
// of past closed-out meetings) as one JSON blob in Upstash Redis -- same
// "one mutable document" pattern as /api/retirement.js, since this is one
// evolving working set plus a log, not an append-only version history like
// /api/analysis.js.
//
// Requires KV_REST_API_URL / KV_REST_API_TOKEN env vars (Upstash for Redis,
// connected to this project via Vercel Storage).

const REDIS_KEY = "meeting-agenda-data";
